...
func (rq *ResponseQuery) QueryMessage() *MessageQuery {
...
func (rq *ResponseQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (rq *ResponseQuery) WithMessage(opts ...func(*MessageQuery)) *ResponseQuery {
	query := (&MessageClient{config: rq.config}).Query()
...
func (rq *ResponseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Response, error) {
	...
	if query := rq.withMessage; query != nil {
		if err := rq.loadMessage(ctx, query, nodes, nil,
			func(n *Response, e *Message) { n.Edges.Message = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withUser; query != nil {
		if err := rq.loadUser(ctx, query, nodes, nil,
			func(n *Response, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResponseQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Response, init func(*Response), assign func(*Response, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Response)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (rq *ResponseQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(response.Table, response.Columns, sqlgraph.NewFieldSpec(response.FieldID, field.TypeInt))
	_spec.From = rq.sql
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withMessage != nil {
			_spec.Node.AddColumnOnce(response.FieldUserID)
		}
		if rq.withUser != nil {
			_spec.Node.AddColumnOnce(response.FieldUserID)
		}
