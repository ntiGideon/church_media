...
func (sq *ServiceQuery) QueryAttendanceRecords() *AttendanceRecordQuery {
	query := (&AttendanceRecordClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withAttendanceRecords: sq.withAttendanceRecords.Clone(),
...
func (sq *ServiceQuery) WithAttendanceRecords(opts ...func(*AttendanceRecordQuery)) *ServiceQuery {
	query := (&AttendanceRecordClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withAttendanceRecords = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withAttendanceRecords; query != nil {
		if err := sq.loadAttendanceRecords(ctx, query, nodes,
			func(n *Service) { n.Edges.AttendanceRecords = []*AttendanceRecord{} },
			func(n *Service, e *AttendanceRecord) {
...
func (sq *ServiceQuery) loadAttendanceRecords(ctx context.Context, query *AttendanceRecordQuery, nodes []*Service, init func(*Service), assign func(*Service, *AttendanceRecord)) error {
	...
	query.Where(predicate.AttendanceRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.AttendanceRecordsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.service_attendance_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_attendance_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_attendance_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
