...
func (mq *MessageQuery) QueryResponses() *ResponseQuery {
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withResponses: mq.withResponses.Clone(),
...
func (mq *MessageQuery) WithResponses(opts ...func(*ResponseQuery)) *MessageQuery {
	query := (&ResponseClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withResponses = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withResponses; query != nil {
		if err := mq.loadResponses(ctx, query, nodes,
			func(n *Message) { n.Edges.Responses = []*Response{} },
			func(n *Message, e *Response) { n.Edges.Responses = append(n.Edges.Responses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadResponses(ctx context.Context, query *ResponseQuery, nodes []*Message, init func(*Message), assign func(*Message, *Response)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(response.FieldUserID)
	}
	query.Where(predicate.Response(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(message.ResponsesColumn), fks...))
	}))
